		...
		$.each(dataArray, function(index, file) {	
			...
			$.ajax({
				...
				/*	filepath = SITEURL+"images/property/"+dataSplit[0];
					...
					$('#uploaded-files').html("<img src = '"+SITEURL+"images/black-001-loading.gif' width = '20' height = '20' >");					
					$('#uploaded-files').load(SITEURL+'myaccount/uploaded', function() {
					
						if(data == 'Maximum limit reached')
					{
						$('#uploaded-files').prepend('<h4>Maximum limit reached</h4>');
//						$('<h4>Maximum limit reached</h4>').insertAfter('#uploaded-files');

					}
					else
					{
						$('#uploaded-files').find('h4').remove();	
