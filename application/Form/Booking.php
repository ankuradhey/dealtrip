...
{  
	...
	{   
		...
		if($bookId != "")
		{
				...
				if($bookArr != "" && count($bookArr)>0)	
				{
					$date_from_value = date('d-m-Y',strtotime($bookArr[0]['date_from']));						
					$adults_value = $bookArr[0]['adult'];
					$childs_value = $bookArr[0]['children'];
					$infants_value = $bookArr[0]['infants'];
				}
		...
		$adults_value = $adults_value?$adults_value:$mySession->Adults;
		$childs_value = $childs_value?$childs_value:$mySession->Children;
		$infants_value = $infants_value?$infants_value:$mySession->Infants;
