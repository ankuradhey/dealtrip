...
//                    }
		...
		if($this->checkAmount($narrivalDate,$dateTo,$dataForm['date_to']+1,$pptyId) === 0)
                    return "Allowed Minimum Nights failed";
		elseif($this->checkAmount($narrivalDate,$dateTo,$dataForm['date_to']+1,$pptyId) == 0)
                    return "There are no rates for this property for the date you have requested - please contact Reservation on <b>0800 180 4889</b> who will assist you.";
 	...
 	public function checkAmount($datefrom,$dateto, $minNights, $pptyId)
	{
		...
		$sum_query = " select * ,
										 coalesce (sum(
											 ...
											 end), (select ceil(min(prate)*exchange_rate)*".$minNights.")) 
		...
		{
			$minrate = $db->runQuery("select ceil(min(prate)*exchange_rate) as MIN from ".CAL_RATE." 
									 inner join ".PROPERTY." on ".PROPERTY.".id = ".CAL_RATE.".property_id
  								     inner join ".CURRENCY." on ".PROPERTY.".currency_code = ".CURRENCY.".currency_code
			                         where property_id = '".$pptyId."' ");
