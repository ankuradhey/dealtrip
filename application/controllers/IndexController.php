...
//            $this->view->headMeta('description', $metaArr[0]['meta_description']);
        ...
        $sbookingyArr = $db->runQuery("select property_title,booking.property_id as property_id,property.propertycode as propertycode,(select image_title from " . GALLERY . " where property_id=booking.property_id order by gallery_id desc limit 0,1) as image_title,(select image_name from " . GALLERY . " where property_id=booking.property_id order by gallery_id desc limit 0,1) as image_name, ptyle_url,
                                                                  " . COUNTRIES . ".country_name, " . STATE . ".state_name, " . CITIES . ".city_name, " . LOCAL_AREA . ".local_area_name, " . SUB_AREA . ".sub_area_name,
                                                                  bedrooms,    trim(trailing '.' from trim(trailing 0 from " . PROPERTY . ".bathrooms)) as bathrooms from " . BOOKING . "
                  ...
                  left join " . LOCAL_AREA . " on " . LOCAL_AREA . ".local_area_id = " . PROPERTY . ".local_area_id 
                                    ...
                                    order by " . BOOKING . ".booking_id desc limit 0,5");

        $this->view->sbookingyArr = $sbookingyArr;
...
//prd($pptyArr);
        ...
        //--------------------------//
        //fetching popular location//
        //--------------------------//

        $poloArr = $db->runQuery("select " . SLIDES_PROPERTY . ".*, " . PROPERTY . ".id, " . PROPERTY . ".propertycode," . PROPERTY . ".property_title, " . PROPERTY . ".brief_desc, " . PROPERTY . ".star_rating, image_name,
                                                                  " . COUNTRIES . ".country_name, " . STATE . ".state_name, " . CITIES . ".city_name, " . LOCAL_AREA . ".local_area_name, " . SUB_AREA . ".sub_area_name,
                                                                   bedrooms,   trim(trailing '.' from trim(trailing 0 from " . PROPERTY . ".bathrooms)) as bathrooms, ptyle_url
                                                                  from " . SLIDES_PROPERTY . "
                                                                  inner join " . PROPERTY . " on " . PROPERTY . ".id = " . SLIDES_PROPERTY . ".lppty_property_id
                                  ...
                                  left join " . GALLERY . " on " . GALLERY . ".property_id = " . SLIDES_PROPERTY . ".lppty_property_id
                                  where lppty_status = '1' 
                                  group by id
                                  order by " . BOOKING . ".booking_id desc limit 0,6 ");

        $countpoplocation = $db->runQuery("select count(*) as _count from (select " . PROPERTY . ".id  from " . SLIDES_PROPERTY . "
                                                                  inner join " . PROPERTY . " on " . PROPERTY . ".id = " . SLIDES_PROPERTY . ".lppty_property_id
                                                                  inner join " . PROPERTY_TYPE . " on " . PROPERTY . ".property_type = " . PROPERTY_TYPE . ".ptyle_id
        ...
        $this->view->countpoplocation = $countpoplocation[0]['_count'];
...
//            prd($this->spclofferpptyAction());
        ...
        $new_sql = "select synonyms, page_title from " . PAGES1 . " where page_parent = 'general' and status=1";
        ...
        $area1PropertyArr = $countryArr = $db->runQuery("select country_name,count(id) as tot, " . COUNTRIES . ".country_id as country_id from " . PROPERTY . " 
                                         inner join " . COUNTRIES . " on " . COUNTRIES . ".country_id = " . PROPERTY . ".country_id
                                         where  " . PROPERTY . ".status= '3' 
                                         group by country_name
                                         ");
        $areaPropertyArr = array();
        $i = 0;

        foreach ($area1PropertyArr as $row1) {

            $areaPropertyArr[$i]['name'] = $row1['country_name'];
            $areaPropertyArr[$i]['url'] = "" . $row1["country_name"];
            $areaPropertyArr[$i]['tot'] = $row1['tot'];
            $areaPropertyArr[$i]['type'] = 1;
            $i++;
            $area2PropertyArr = $db->runQuery("select country_name,state_name, " . STATE . ".state_id as state_id,count(id) as tot from " . PROPERTY . "
                                         inner join " . COUNTRIES . " on " . COUNTRIES . ".country_id = " . PROPERTY . ".country_id
                                         inner join " . STATE . " on " . STATE . ".state_id = " . PROPERTY . ".state_id
                                         where  " . PROPERTY . ".status= '3'  and " . STATE . ".country_id=" . $row1['country_id'] . "
                                         group by state_name
                                         ");
            if (count($area2PropertyArr) > 0) {
                foreach ($area2PropertyArr as $row2) {
                    $areaPropertyArr[$i]['name'] = $row2['state_name'];
                    $areaPropertyArr[$i]['url'] = "" . $row2["country_name"] . "/" . $row2['state_name'];
                    $areaPropertyArr[$i]['tot'] = $row2['tot'];
                    $areaPropertyArr[$i]['type'] = 2;
                    $i++;

                    $area3PropertyArr = $db->runQuery("select  country_name, state_name, city_name," . CITIES . ".city_id as city_id,count(id) as tot from " . PROPERTY . " 
                                         inner join " . COUNTRIES . " on " . COUNTRIES . ".country_id = " . PROPERTY . ".country_id
                                         inner join " . STATE . " on " . STATE . ".state_id = " . PROPERTY . ".state_id
                                         inner join " . CITIES . " on " . CITIES . ".city_id = " . PROPERTY . ".city_id
                                         where  " . PROPERTY . ".status= '3'  and " . CITIES . ".state_id=" . $row2['state_id'] . "
                                         group by city_name
                                         ");
                    if (count($area3PropertyArr) > 0) {
                        foreach ($area3PropertyArr as $row3) {
                            $areaPropertyArr[$i]['name'] = $row3['city_name'];
                            $areaPropertyArr[$i]['url'] = "" . $row3["country_name"] . "/" . $row3['state_name'] . "/" . $row3['city_name'];
                            $areaPropertyArr[$i]['tot'] = $row3['tot'];
                            $areaPropertyArr[$i]['type'] = 3;
                            $i++;
                            $area4PropertyArr = $db->runQuery("select country_name, state_name, city_name,sub_area_name," . SUB_AREA . ".sub_area_id as sub_area_id,count(id) as tot from " . PROPERTY . " 
                                         inner join " . COUNTRIES . " on " . COUNTRIES . ".country_id = " . PROPERTY . ".country_id
                                         inner join " . STATE . " on " . STATE . ".state_id = " . PROPERTY . ".state_id
                                         inner join " . CITIES . " on " . CITIES . ".city_id = " . PROPERTY . ".city_id
                                         inner join " . SUB_AREA . " on " . SUB_AREA . ".sub_area_id = " . PROPERTY . ".sub_area_id
                                         where " . PROPERTY . ".status= '3' and " . SUB_AREA . ".city_id=" . $row3['city_id'] . "
                                         group by sub_area_name
                                         ");
                            if (count($area4PropertyArr) > 0) {
                                foreach ($area4PropertyArr as $row4) {
                                    $areaPropertyArr[$i]['name'] = $row4['sub_area_name'];
                                    $areaPropertyArr[$i]['url'] = "" . $row4["country_name"] . "/" . $row4['state_name'] . "/" . $row4['city_name'] . "/" . $row4['sub_area_name'];

                                    $areaPropertyArr[$i]['tot'] = $row4['tot'];
                                    $areaPropertyArr[$i]['type'] = 4;
                                    $i++;
                                    $area5PropertyArr = $db->runQuery("select country_name, state_name, city_name,sub_area_name, local_area_name,count(id) as tot  from " . PROPERTY . " 
                                         inner join " . COUNTRIES . " on " . COUNTRIES . ".country_id = " . PROPERTY . ".country_id
                                         inner join " . STATE . " on " . STATE . ".state_id = " . PROPERTY . ".state_id
                                         inner join " . CITIES . " on " . CITIES . ".city_id = " . PROPERTY . ".city_id
                                         inner join " . SUB_AREA . " on " . SUB_AREA . ".sub_area_id = " . PROPERTY . ".sub_area_id
                                         inner join " . LOCAL_AREA . " on " . LOCAL_AREA . ".local_area_id = " . PROPERTY . ".local_area_id
                                         where  " . PROPERTY . ".status= '3'  and " . LOCAL_AREA . ".sub_area_id=" . $row4['sub_area_id'] . "
                                         group by local_area_name
                                         ");
                                    if (count($area5PropertyArr) > 0) {
                                        foreach ($area5PropertyArr as $row5) {
                                            $areaPropertyArr[$i]['name'] = $row5['local_area_name'];
                                            $areaPropertyArr[$i]['url'] = "" . $row5["country_name"] . "/" . $row5['state_name'] . "/" . $row5['city_name'] . "/" . $row5['sub_area_name'] . "/" . $row5['local_area_name'];
                                            $areaPropertyArr[$i]['tot'] = $row5['tot'];
                                            $areaPropertyArr[$i]['type'] = 5;
                                            $i++;
                                        }
                                    }
                                }
        ...
        $this->view->areaPropertyArr = $areaPropertyArr;





        $this->view->poloArr = $poloArr;
    }
...
//                            echo '<div class="tab_conten_clmn2">';
                    ...
                    echo'<div class="tab_box_text">
                        <h3><a class="tab_box_heading" href="' . $url . '" >' . $values['property_title'] . '</a>
                                            </h3>
                                          <div class="tab_box_pic">' . ppty_image_display($values['image_name']) . '</div>
                                          <div class="tab_box_detail">
                                            <div class="tab_box_dis">' . $values['brief_desc'] . '</div>';
...
//                            echo '<div class="tab_conten_clmn2">';
                    ...
                    echo '<div class="tab_box_text">
                            <h3><a class="tab_box_heading" href="' . $url . '">' . $values['property_title'] . '</a></h3>
                                          <div class="tab_box_pic">' . ppty_image_display($values['image_name']) . '</div>
                                          <div class="tab_box_detail">
...
//                                echo '<div class="tab_conten_clmn2">';
            ...
            case '4': //query for review 
                ...
                $poloArr = $db->runQuery("select " . SLIDES_PROPERTY . ".*, " . PROPERTY . ".id, " . PROPERTY . ".propertycode," . PROPERTY . ".property_title, " . PROPERTY . ".brief_desc, " . PROPERTY . ".star_rating, image_name,
                                  ...
                                  where lppty_status = '1' 
                                  group by id
                                  order by " . BOOKING . ".booking_id desc limit $limit,6 ");


                $i = 1;

                foreach ($poloArr as $values) {
