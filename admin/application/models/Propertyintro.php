        ...
        public function savePropertyintro($dataForm, $ptyleId = "")
        {
            ...
            if ($ptyleId == "")
            {
                ...
                if ($dataForm['library_save'])
                {
                    ...
                    if (!empty($dataForm['sub_area_id']))
                        $query .= "inner join " . SUB_AREA . " on " . SUB_AREA . ".sub_area_id = " . PROPERTY . ".sub_area_id";

                    if (!empty($dataForm['local_area_id']))
                        $query .= "inner join " . LOCAL_AREA . " on " . LOCAL_AREA . ".local_area_id = " . PROPERTY . ".local_area_id";
...
//                    $data_update['local_area_id'] = $dataForm['local_area'];
                    $query .= " where " . PROPERTY . ".country_id = '" . $dataForm['country_id'] . "' and 
                                 " . PROPERTY . ".state_id = '" . $dataForm['state_id'] . "' and
                                 " . PROPERTY . ".city_id = '" . $dataForm['city_id'] . "' and
                                 " . PROPERTY . ".sub_area_id = '" . $dataForm['sub_area'] . "' and
                                 " . PROPERTY . ".local_area_id = '" . $dataForm['local_area'] . "' and
                                 " . PROPERTY . ".property_type = '" . $dataForm['accomodation_type'] . "' and
                                 " . PROPERTY . ".status = '5'    
                        ";

                    //prd($query);

                    $chkQry = $db->runQuery($query);
...
//                    prd($dataForm);
                    ...
                    $data_update['user_id'] = 11;

                    $data_update['meta_keywords'] = addslashes($dataForm['meta_keywords']);
                    $data_update['meta_description'] = addslashes($dataForm['meta_description']);

                    if (empty($data_update['meta_keywords']))
                        $data_update['meta_keywords'] = $dataForm['title'] . ", " . $dataForm['no_of_bedroom'] . " bedrooms, " . $dataForm['no_of_bathroom'] . " bathrooms, " . $dataForm['ppty_no'];

                    if (empty($data_update['meta_description']))
                        $data_update['meta_description'] = substr($dataForm['introduction'], 0, 200);

//                    prd($data_update);
                    $db->save(PROPERTY, $data_update);
                    $ptyleId = $mySession->admin_property_id = $db->lastInsertId();

                    $mySession->step = '1';
                    
                    
                    //TO DO - enable once tested
//                    if(0){
                        //save attribute priming values
                        $dataAttribute = array();
                        foreach($dataForm['primes'] as $primeKey=>$primeVal){
                            $dataAttribute['ans_attribute_id'] = $primeVal;
                            $dataAttribute['ans_property_id'] = $ptyleId;
                            $db->save(ATTRIBUTE_ANS, $dataAttribute);
                        }
//                    }
                    
                    //save custom attributes
                    foreach($dataForm['customAttribute'] as $customVal){
                        $customAttribute = array();
                        $customAttribute['ans_attribute_id'] = '0';
                        $customAttribute['ans_property_id'] = $ptyleId;
                        $customAttribute['ans_custom_value'] = $customVal;
                        $res = $db->save(ATTRIBUTE_ANS, $customAttribute);
                    }
                    return 1;
                }
            }
            else
            {
                ...
                if ($dataForm['step'] == "1")
                {
                    ...
                    foreach($dataForm['primes'] as $primeKey=>$primeVal){
                        ...
                        $primesArr[] = $primeVal;
                        //check old entry
                        $condition = " ans_property_id = '" . $ptyleId . "' and ans_attribute_id = '" . $primeVal. "' ";
                        $chkQry = $db->runQuery("select * from ".ATTRIBUTE_ANS." where $condition ");
                        if (empty($chkQry) || count($chkQry) == 0){
                            $dataAttribute['ans_attribute_id'] = $primeVal;
                            $dataAttribute['ans_property_id'] = $ptyleId;
                            $res = $db->save(ATTRIBUTE_ANS, $dataAttribute);
                        }
                    }
                    
//                    $primesArr = array_diff($primesArr, $customAttribute)
                    //save custom attributes
                    foreach($dataForm['customAttribute'] as $customKey=>$customVal){
                        $customAttribute = array();
                        $customAttribute['ans_custom_value'] = $customVal;
                        $customPrimesArr[] = $dataForm['customAttributeId'][$customKey];
                        //modify custom attribute
                        if(!empty($dataForm['customAttributeId'][$customKey])){
                            if(!empty($customVal))
                            $res = $db->modify(ATTRIBUTE_ANS, $customAttribute, " ans_id = ".$dataForm['customAttributeId'][$customKey]);
                            else {
                                $db->delete(ATTRIBUTE_ANS," ans_id = ".$dataForm['customAttributeId'][$customKey]);
                            }
                        }else{
                            $customAttribute['ans_attribute_id'] = '0';
                            $customAttribute['ans_property_id'] = $ptyleId;
                            $res = $db->save(ATTRIBUTE_ANS, $customAttribute);
                            $customPrimesArr[] = $db->lastInsertId();
                        }
                    }
                    //remove unchecked attribute
                    if(!empty($primesArr) && !empty($customPrimesArr)){ 
                        $primesArr = implode(",",$primesArr);
                        $customPrimesArr = implode(",",$customPrimesArr);
                        $db->delete(ATTRIBUTE_ANS, " ans_property_id = '$ptyleId' and ans_id not in ($customPrimesArr) and ans_attribute_id not in ($primesArr) ");
                    }elseif(!empty($primesArr)){
                        $primesArr = implode(",",$primesArr);
                        $db->delete(ATTRIBUTE_ANS, " ans_property_id = '$ptyleId' and  ans_attribute_id not in ($primesArr) ");
                    }elseif(!empty($customPrimesArr)){
                        $customPrimesArr = implode(",",$customPrimesArr);
                        $db->delete(ATTRIBUTE_ANS, " ans_property_id = '$ptyleId' and ans_id not in ($customPrimesArr) ");
                    }else{
                        $db->delete(ATTRIBUTE_ANS, " ans_property_id = '$ptyleId'");
                    }
                }
                ...
                if ($dataForm['step'] == "2")
                {
                    ...
                    $bathroomArr = $db->runQuery("select * from  " . PROPERTY . " where id = '" . $ptyleId . "' ");
                    $data_update = array();
                    ...
                    foreach ($specArr as $values)
                    {
                        ...
                        if (!is_array($dataForm[$ques]) || $values['spec_id'] == ADDITIONAL_BATHROOM_ID)
                        {
                            if (isset($dataForm[$ques]) && $dataForm[$ques] != "")
                            {
                                ...
                                $chkSpec = $db->runQuery("select * from " . SPEC_ANS . " where property_id = '" . $ptyleId . "'  and spec_id = '" . $values['spec_id'] . "'");
                                if (count($chkSpec) > 0 && $chkSpec != "")
                                {
                                    $condition = " property_id = '" . $ptyleId . "' and spec_id = '" . $values['spec_id'] . "' ";
                    ...
                    {
                        $chkStat = $db->runQuery("select * from " . SPEC_ANS . " where property_id = '" . $ptyleId . "' and spec_id = '" . $sval['spec_id'] . "'");
                ...
                if ($dataForm['step'] == "3")
                {
                    ...
                    $chkAmenity = $db->runQuery("select * from " . AMENITY_ANS . " where property_id = '" . $ptyleId . "' ");
                    ...
                    $statusArr = $db->runQuery("select * from " . PROPERTY . " where id = '" . $ptyleId . "' and big_desc != '' and airport1 != ''");
                ...
                if ($dataForm['step'] == "4")
                {
                    ...
                    $statusArr = $db->runQuery("select * from " . PROPERTY . " where id = '" . $ptyleId . "' and address != '' and cletitude != '' and clongitude != '' ");
                    if ($statusArr != "" && count($statusArr) > 0)
                    {
                ...
                /* if($dataForm['step'] == "8")
                  ...
                  $checkQuery = $db->runQuery("select * from ".OWNER_REVIEW." where property_id = '".$ptyleId."' ");
                ...
                if ($dataForm['step'] == "9")
                {
                    ...
                    $checkQuery = $db->runQuery("select * from " . PROPERTY . " where id = '" . $ptyleId . "' ");
        ...
        public function copyProperty($ppty_no, $param1, $param2, $param3, $param4)
        {
            ...
            $data_update['user_id'] = 11;   //user id added for admin
            $data_update['propertycode'] = generate_property_no("11");
            ...
            if ($chkQuery[0]['floor_plan'] != "")
            {
                ...
                $randomname = date() . time() . $tmp_name[count($tmp_name) - 1];
                copy(SITE_ROOT . "images/floorplan/" . $checkQuery[0]['floor_plan'], SITE_ROOT . "images/floorplan/" . $randomname);
                $data_update['floor_plan'] = $randomname;
            }

            //step 6
            $data_update['cal_default'] = $chkQuery[0]['cal_default'];
            ...
            if ($chkQuery[0]['arrival_instruction'] != "")
            {
                ...
                $randomname = date() . time() . $tmp_name[count($tmp_name) - 1];
                copy(SITE_ROOT . "images/uploads/instructions/" . $checkQuery[0]['arrival_instruction'], SITE_ROOT . "images/uploads/instructions/" . $randomname);
            ...
            $data_update['meta_keywords'] = $chkQuery[0]['meta_keywords'];
            $data_update['meta_description'] = $chkQuery[0]['meta_description'];

            //prd($data_update);
            $db->save(PROPERTY, $data_update);
            $ptyleId = $db->lastInsertId();
            $mySession->ppty_no = $data_update['propertycode'];
            //$mySession->step = '1';			
            
            //COPY ATTRIBUTES
            $attributes = $db->runQuery(" select * from ".ATTRIBUTE_ANS." where ans_property_id = '".$chkQuery[0]['id']."' ");
            foreach($attributes as $attr){
                $data_attr = array();
                $data_attr['ans_attribute_id'] = $attr['ans_attribute_id'];
                $data_attr['ans_property_id'] = $ptyleId;
                $data_attr['ans_custom_value'] = $attr['ans_custom_value'];
                $db->save(ATTRIBUTE_ANS, $data_attr);
            }
            ...
            foreach ($specArr as $val)
            {
                ...
                $data_update1['property_id'] = $ptyleId;
            ...
            $extrasArr = $db->runQuery("select * from " . EXTRAS . " where property_id = '" . $chkQuery[0]['property_id'] . "' ");
            $data_update1 = array();
            ...
            $chkstatusArr = $db->runQuery("select * from " . PROPERTY . " where id = '" . $ptyleId . "' ");
