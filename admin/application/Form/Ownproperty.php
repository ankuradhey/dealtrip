<?PHP
    ...
    class Form_Ownproperty extends Zend_Form
    {
        ...
        public function init($ppty_id)
        {
            ...
            $arrival_instruction_value = "";
            $title_value = "";
            $introduction_value = "Up to 300 Characters";
            $sub_area_value = "";
            $local_area_value = "";
            ...
            if ($ppty_id != "")
            {
                ...
                $attributeVal = $db->runQuery("select attrib.attribute_id, attrib.attribute_name from ".ATTRIBUTE_ANS." attrib_ans inner join ".ATTRIBUTE." attrib on attrib_ans.ans_attribute_id = attrib.attribute_id where attribute_status = '1' and attrib_ans.ans_property_id= '$ppty_id' ");
                $attributeVal = array_map(function($x){
...
//		echo $_REQUEST['country_id']; exit; 
             ...
             */
            $primesData = $db->runQuery("select * from " . ATTRIBUTE . " where attribute_status = '1' ");
            
            foreach($primesData as $primesKey=>$primesVal){
                $primesArr[$primesKey]['key'] = $primesVal['attribute_id'];
                $primesArr[$primesKey]['value'] = $primesVal['attribute_name'];
            }
            $primes = new Zend_Form_Element_MultiCheckbox('primes');
            $primes->addMultiOptions($primesArr)
            ->setAttrib("class","mws-textinput")
            ...
            $ppty_no->setValue(generate_property_no(11));

            $this->addElements(array($title, $introduction, $country_id, $state_id, $city_id, $zipcode, $accomodation_type, $no_of_bedroom, $no_of_bathroom, $no_of_en_suite_bathroom, $occupancy, $step, $sub_area, $local_area, $ppty_no, $meta_keywords, $meta_description, $primes));
        }
