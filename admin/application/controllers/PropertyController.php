...
//			  inner join " . USERS . "  as u on u.user_id = p.user_id where p.status != '0' and p.status != '5'
//                          and u.user_id != '11'    
//			  order by p.id  ";
        ...
        public function ajaxlistAction(){
			...
			$searchCondition=" ( propertycode like '%".$searchValue."%' or  subscriber_name  like '%".$searchValue."%' or xml_property_id like '%".$searchValue."%' or ptyle_name like '%".$searchValue."%' )";
            $start = $data['start'];
            $offset = $data['length'];
            $db = new Db();
            $returnArr = array();
            $resultArr = array();
            $totalRecords = "select count(*) as count from " . PROPERTY . " as p
                    inner join " . PROPERTY_TYPE . " as pt on pt.ptyle_id = p.property_type
                    left join subscriber as s on s.subscriber_id = p.xml_subscriber_id
                    inner join " . USERS . "  as u on u.user_id = p.user_id 
                    where ( p.status  != '5' and p.status != '0') and u.user_id = '11'
                     and " .$searchCondition;
            $tresData = $db->runQuery($totalRecords);
            $totalRecords = $tresData[0]['count'];
            $qry = "select * from " . PROPERTY . " as p
                    inner join " . PROPERTY_TYPE . " as pt on pt.ptyle_id = p.property_type
                    left join subscriber as s on s.subscriber_id = p.xml_subscriber_id
                    inner join " . USERS . "  as u on u.user_id = p.user_id 
                    where ( p.status  != '5' and p.status != '0') and u.user_id = '11' and ".$searchCondition."
                    order by p.id desc
                    limit $start,$offset
                    ";
					//search[value] 
            ...
            foreach($resData as $key=>$res){
                $resultArr[$key][0] = $start+$key+1;
                $resultArr[$key][1] = "<input type='checkbox' value='".$res['id']."' name='check.$key' id='check.$key' onchange='return check_check(\"bcdel\",\"deletebcchk\")' />";
                $resultArr[$key][2] = $res['propertycode'];
                $resultArr[$key][3] = $res['subscriber_name'];
                $resultArr[$key][4] = $res['xml_property_id'];
                $resultArr[$key][5] = $res['ptyle_name'];
                $resultArr[$key][6] = date('d-m-Y', strtotime($res['date_added']));
                switch ($res['status'])
                        {
                ...
                $resultArr[$key][7] = '<select class="chzn-select" name = "status'.$key.'" id = "status'.$i.'" onchange="changestatus('.$res['id'].',this.value)">
                ...
                $resultArr[$key][8] = "<a href='".APPLICATION_URL_ADMIN."property/editproperty/pptyId/".$res['id']."/step/0'>
                    <img src='".APPLICATION_URL_ADMIN."css/icons/32/application_form_edit.png' height='16' width='16'   border='0' title='Edit' alt='Edit'></a>";
            }
            //$returnArr['draw'] = $start+1;
            $returnArr['iTotalRecords'] = $totalRecords;
            $returnArr['iTotalDisplayRecords'] = $totalRecords;
            $returnArr['data'] = $resultArr;
            
            echo $this->_helper->json($returnArr);
            die;
        }
        
        public function ajaxpropertylistAction(){
            $data = $this->getRequest()->getParams();
            $start = $data['start'];
            $offset = $data['length'];
            $db = new Db();
            ...
            foreach($resData as $key=>$res){
                ...
                $resultArr[$key][1] = "<input type='checkbox' value='".$res['id']."' name='check.$key' id='check.$key' onchange='return check_check(\"bcdel\",\"deletebcchk\")' />";
                $resultArr[$key][2] = $res['propertycode'];
                $resultArr[$key][3] = $res['first_name']." ".$res['last_name'];
                $resultArr[$key][4] = $res['ptyle_name'];
                $resultArr[$key][5] = date('d-m-Y', strtotime($res['date_added']));
                switch ($res['status'])
                        {
                            case '1': $status1 = 'selected="selected"';
                                $status2 = "";
                ...
                $resultArr[$key][6] = '<select class="chzn-select" name = "status'.$key.'" id = "status'.$i.'" onchange="changestatus(\"'.$row['id'].'\",this.value)">
                                    <option '.$status1.' value="1">Incomplete</option>
                                    <option '.$status2.' value="2">Pending Approval</option>                                    
                                    <option '.$status3.' value="3">Live</option>                                    
                                    <option '.$status4.' value="4">Suspended</option>                                    
                                </select>';
                $resultArr[$key][7] = "<a href='".APPLICATION_URL_ADMIN."property/editproperty/pptyId/".$res['id']."/step/0'>
                    <img src='".APPLICATION_URL_ADMIN."css/icons/32/application_form_edit.png' height='16' width='16'   border='0' title='Edit' alt='Edit'></a>";
            }
            $returnArr['draw'] = $start+1;
            $returnArr['recordsTotal'] = $totalRecords;
            $returnArr['recordsFiltered'] = $totalRecords;
            $returnArr['data'] = $resultArr;
        ...
        {
            global $mySession;
            $this->view->pageHeading = "Manage Property Attributes";
            $db = new Db();
            $qry = "select * from " . ATTRIBUTE . " as attr ";
            $order = "order by attribute_id desc";
            $this->view->arrData = $db->runQuery("$qry $where $order");
        }

        #-----------------------------------------------------------#
        # Add Property Attribute Action Function
        #-----------------------------------------------------------#
        ...
        public function addattributesAction()
        {
            ...
            if ($this->getRequest()->isPost())
            {
                    ...
                    $dataForm = $_REQUEST;
                    $myObj = new Attribute();
                    $result = $myObj->saveAttribute($dataForm);
                    if ($result == 1)
                    {
                        $mySession->sucessMsg = "New Property Attribute Added Sucessfully";
                        $this->_redirect('property/addattributes');
                    }
                    else
                    {
                        $mySession->errorMsg = "This Attribute Already Exists.";
                        $this->view->myform = $myform;
                    }
            }
        }
        ...
        #-----------------------------------------------------------#
        # Attribute Delete Action Function
        #-----------------------------------------------------------#
            ...
            if ($_REQUEST['Id'] != "")
            {
                ...
                if (count($arrId) > 0)
                {
                    ...
                    foreach ($arrId as $key => $Id)
                    {
                        ...
                        {
                            $myObj = new Attribute();
                            $Result = $myObj->deleteAttribute($Id);
                        }
                    }
                }
            }
        ...
        #-----------------------------------------------------------#
        # Change Property Attribute Status Action Function
        #-----------------------------------------------------------#

        public function changeattrstatusAction()
        {
            global $mySession;
            $db = new Db();
            $specId = $_REQUEST['Id'];
            ...
            $myObj = new Attribute();
            $Result = $myObj->statusAttribute($status, $specId);
            exit();
        }
        ...
        #-----------------------------------------------------------#
        # Property Attribute Edit Action Function
        #-----------------------------------------------------------#
        ...
        public function editpropertyAction()
        {
            ...
            $ratingArr = $db->runQuery("select * from " . PROPERTY . " 
                                        inner join " . PROPERTY_TYPE . " on " . PROPERTY_TYPE . ".ptyle_id = " . PROPERTY . ".property_type
                                        where id = '" . $pptyId . "' ");

            $preview_link_data = $db->runQuery("select " . PROPERTY . ".propertycode ," . COUNTRIES . ".country_name, " . STATE . ".state_name, " . CITIES . ".city_name, 
                " . LOCAL_AREA . ".local_area_name, " . SUB_AREA . ".sub_area_name,ptyle_name, ptyle_url, bedrooms, bathrooms, is_insured  ,
                    coalesce(local_area_name, sub_area_name, city_name) as location_name
                    from " . PROPERTY . " 
                                                    inner join " . PROPERTY_TYPE . " on " . PROPERTY_TYPE . ".ptyle_id = " . PROPERTY . ".property_type
            ...
            switch ($stepId)
            {
                ...
                case '5': $myform = new Form_Cal();
                    ...
                    $this->view->date_calendar_modified = $pptyArr[0]['date_calendar_modified'];
                    break;
            ...
            $this->view->propertyData = $ratingArr[0];
            $this->view->myform = $myform;
        }
...
//                    prd($dataForm);
        ...
        public function savecalendarstatAction()
        {
            ...
            //saving date for updated calendar
            $data_array = array();
            $data_array['date_calendar_modified'] = date('Y-m-d');

            if ($pptyId != "")
            {
                $db->modify(PROPERTY, $data_array, " id =  $pptyId ");
            }
            else
            {
                $db->modify(PROPERTY, $data_array, " id =  '" . $mySession->property_id . "' ");
            }
        ...
        public function savereviewAction()
        {
            ...
            $pptyId = $this->getRequest()->getParam("pptyId");
            {
                ...
                if (empty($_REQUEST['Name']) || (empty($_REQUEST['Comment']) && empty($_REQUEST['Review'])))
                    $this->_redirect("property/editproperty/pptyId/" . $pptyId . "/step/7");
        ...
        public function processrentalAction()
        {
            ...
            {
                if ($_REQUEST['rental_ques'] != "" || $_REQUEST['is_insured'] == 'true')
                {
                    $dataForm = array();
                    $dataForm['is_insured'] = '0';

                    if ($_REQUEST['rental_ques'] != "")
                        $dataForm['rental_ques'] = $_REQUEST['rental_ques'];
                    if ($_REQUEST['is_insured'] == 'true')
                        $dataForm['is_insured'] = '1';

                    $condition = "id = " . $pptyId;
                    $db->modify(PROPERTY, $dataForm, $condition);
                }
                elseif($_REQUEST['is_insured']=='false')
                {
                    $dataForm = array();
                    $dataForm['is_insured'] = '0';
                    if ($_REQUEST['rental_ques'] != "")
            ...
            if ($pptyId != "")
                $this->_redirect("property/editproperty/pptyId/" . $pptyId . "/step/6");
            else
...
//                $this->_redirect("myaccount/addproperty");
        ...
        /*
         * Description - used for saving update calendar time
         */
        function uptodateAction(){
            global $mySession;
            $db = new Db();
            
            $pptyId = $this->_getParam('ppty');
            $type = $this->_getParam('type');
            
            $data = array();
            $data['date_calendar_modified'] = date('Y-m-d');
            
            $db->modify(PROPERTY, $data, ' id = '.$pptyId);
