$(document).ready(function() {
	...
	$(document).on("change",'#photo_res',function(e) {
		...
		$.ajaxFileUpload({
				...
				url: SITEURLADMIN+'property/doajaxfileupload',
				secureuri:false,
				fileElementId:'photo_res',
				dataType: 'json',
				data:{name:'logan', id:PPTY_ID},
				   ...
				   dataObj = data;
					...
					else
					{
						...
						if(data.success == '1') {
							...
							z = z+40;
							image = SITEURLADMIN+"ankit_ups/"+data.name;	
	...
	$("#photo_res").unbind("change");
		...
		$.ajax({
				url: SITEURLADMIN+'property/doajaxfileupload/delete/1',
				data:{id:PPTY_ID},
				success: function (data, status)
				{
	...
	$('#upload-button .upload').click(function() {
		...
		$.each(dataArray, function(index, file) {	
			...
			$.ajax({
				...
				type: 'POST',
				url: SITEURL+'myaccount/upload/pptyId/'+PPTY_ID,
				async:false,
				data: ({name:file}),
				success: function(data){
					...
					if(dataSplit[1] == 'uploaded successfully')
					{
						...
						$.ajax({
								type: 'GET',
								url: SITEURL+'myaccount/uploaded/pptyId/'+PPTY_ID,
								async:false,
								success: function(data){
									data = $.trim(data);
									tmp = data.split("+++");
							...
							//$('#uploaded-files').each(function(index) {
								...
								//$('#uploaded-files div').each(function(index) {
									...
									for(var i=0;i<tmp.length;i++)
									{
											...
											if($.trim(tmp1[2]) == "")
											{	
												var title = "No Caption Set -- To set <a href='"+SITEURL+"property/setcaption/id/"+tmp1[0]+"' id = 'email_link"+i+"' >Click here</a>";
											...
											{	
												var title = tmp1[2]+"<br><a href='"+SITEURL+"property/setcaption/id/"+tmp1[0]+"' id = 'email_link"+i+"' >Click here to change</a>";
												$("#img_blck"+i).removeClass("cancel");
											}
				...
				/*	filepath = SITEURL+"images/property/"+dataSplit[0];
						 
					var realData = '<li><a href="images/'+dataSplit[0]+'"><img src="'+filepath+'" border = "0" height="50" width="50"></a> '+dataSplit[1]+'</li>';
					$('#uploaded-files').append('<li><a href="images/'+dataSplit[0]+'"><img src="'+filepath+'" border ="0" height="50" width="50" ></a> Y'+dataSplit[1]+'</li>');
						...
						$('#uploaded-files').append('<li><a href="images/'+data+'. File Name: '+file.name+'</li>');
					}*/
	...
	$('#dropped-files #upload-button .delete').click(function(){
		restartFiles();
		$('#uploaded-files').show();
